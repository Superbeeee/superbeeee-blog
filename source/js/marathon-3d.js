...
/* ── 色階 ───────────────────────────────────────────
 * 通則：連續量＝單一色相（深→淺）；極性（比基準快還是慢）＝兩個色相 +
 * 灰色中點。連續量不用彩虹色階 —— 內插會在資料裡造出根本不存在的分界。
 *
 * 連續的那兩組（RAMP_ORANGE、RAMP_DIVERGING 兩臂）用 dataviz 驗證器對本頁
 * 實際底色 #14171c 跑過 ordinal 四項檢查（明度單調、相鄰 ΔL ≥ 0.06、
 * 最暗端對比 ≥ 2:1、單一色相），皆通過。
 *
 * 心率區間是刻意的例外，理由與量到的代價寫在 RAMP_ZONE 上面。
 */
...
/**
 * 心率區間：跑者慣例的綠→黃→紅。這是刻意的例外，記錄一下量到的代價 ——
 * 綠到紅在 OKLCH 上只跨約 150° 的暖色弧，五格擠進去必然相鄰難分。
 * 對本頁底色 #14171c 做過網格搜尋（色相起訖 × 間距指數 × 明度斜坡），
 * 這組是可得的最佳解，實測最差的相鄰配對：
 *   正常視覺 ΔE 11.9（門檻 15）、綠色盲 ΔE 6.8（門檻 8，6–8 屬警告帶）
 * 兩項都沒過。過不了不是調得不夠好 —— 瓶頸永遠是黃↔橘那一對，
 * 中間沒有色相可以插了。傳統 Garmin 配色更糟（綠色盲下 ΔE 2.2）。
 *
 * 因此靠兩個附加通道扛：
 * 1. 明度單調遞減（對比 11.7 → 3.3），色相塌掉時順序還讀得出來
 * 2. 圖例直接標 Z1–Z5 與 bpm 範圍，滑過任一點也看得到實際心率
 *    —— 顏色不是唯一的身分依據
 */
const RAMP_ZONE = ['#2eebb6', '#7bcd41', '#b59c1b', '#c66d15', '#c82c4a'];
    ...
    build(track) {
      ...
      return {
        ...
        legend: {
          ...
          items: RAMP_ZONE.map((c, i) => ({ color: c, label: 'Z' + (i + 1), sub: bands[i] })),
        },
      };
    },
  },
